...
# PASSO 4: boxplot
kgr_maps |>
  ggplot2::ggplot(ggplot2::aes(x = year_epoca_label, y = xco2_anomalia)) +
  ggplot2::geom_boxplot(fill = "gray") +
  ggplot2::labs(
    title = "Distribuição de anomalia por Ano e Época",
    x = "Ano e Época",
    y = "Anomalia"
  ) +
  ggplot2::theme_bw() +
...
kgr_maps |> 
  ...
  ggplot(aes(year_epoca_label,xco2_anomalia)) +
...
map(unique(kgr_maps$year_epoca_label), ~{
  ...
  hotspots <- kgr_maps |>
    ...
    dplyr::slice_max(xco2_anomalia, n = 5, with_ties = FALSE) |>
    dplyr::mutate(tipo = "hotspot")  
  
  coldspots <- kgr_maps |>
    dplyr::filter(year_epoca_label == .x) |> 
    dplyr::slice_min(xco2_anomalia, n = 5, with_ties = FALSE) |>
  ...
  kgr_maps |> 
    ...
    ggplot2::geom_tile(ggplot2::aes(fill = xco2_anomalia)) +
    ggplot2::geom_point(data = pontos_destaque,
               ggplot2::aes(x = longitude, y = latitude, color = tipo),
               shape = 21, size = 3, fill = "white", stroke = 1.2) +
    ggplot2::scale_color_manual(values = c("hotspot" = "red", "coldspot" = "blue")) +
    ggplot2::geom_point(
      data = kgr_maps |> 
        dplyr::filter(year_epoca_label == .x, xco2_anomalia > 0) |>
        dplyr::summarise(
          lon_centro = sum(longitude * xco2_anomalia, na.rm = TRUE) / sum(xco2_anomalia, na.rm = TRUE),
          lat_centro = sum(latitude  * xco2_anomalia, na.rm = TRUE) / sum(xco2_anomalia, na.rm = TRUE)
        ),
      ggplot2::aes(x = lon_centro, y = lat_centro),  # <- atenção: eixos estão trocados no ggplot
      color = "cyan", size = 2.5, shape = 4, stroke = 1.2) +
    viridis::scale_fill_viridis(option = "inferno") +
    ggplot2::coord_equal(
      xlim = c(-59.7700, -49.8361),   # latitude (x no seu plot)
      ylim = c(-12.3561, -1.6058),  # longitude (y no seu plot)
      expand = FALSE
    ) +
    ggplot2::facet_wrap(~ year_epoca_label) +
    ggplot2::labs(
      x = "Longitude", y = "Latitude", fill = "xco2_anomalia",
      title = .x
    ) +
    ggplot2::theme_bw()
})
...
kgr_maps |> 
  ...
  summarise(
    anomaly_xco2_mean = mean(xco2_anomalia, na.rm=TRUE),
    .groups = "drop"
  ) |> 
  ggplot(aes(x=longitude, y=latitude)) +
